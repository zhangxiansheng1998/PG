...
if (querys == '{{threeDSReferenceId1}}') {
	diagnosticBody = JSON.parse(JSON.stringify(diagnosticBody).replace("{{threeDSReferenceId1}}", vars.get("threeDSReferenceId1")));
}
...
function formatDateSubtraction(date) {
	...
	if (timezones.indexOf("8") > -1) {
		log.info("XQY No need to change the query time");
		timeStr = new Date(date.getTime() - 10 * 60000);
	}
	else {
		log.info("XQY need to change the query time");
		timeStr = new Date(date.getTime() - 10 * 60000 + 8 * 60 * 60000);
	}
...
function formatDateAddition(date) {
	...
	if (timezonea.indexOf("8") > -1) {
		...
		log.info("XQY need to change the query time");
		timeStr = new Date(date.getTime() + 5 * 60000 + 8 * 60 * 60000);
	}
